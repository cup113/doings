...
The app pushes two event types (`new_image` and `delete_image`) to all connected clients. Server-Sent Events (`text/event-stream`) was chosen over WebSockets because the communication is strictly one-way (server → client) and SSE is simpler — no handshake protocol, no library, just `EventEmitter` + `Readable.toWeb`.
