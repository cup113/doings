<script lang="ts">
  ...
  import { currentRoom, viewingUser } from '$lib/stores/app';
  ...
  $effect(() => {
    ...
    loading = true;
    fetchError = '';
    fetchWithRetry(() => fetchRecentImages(r)).then((result) => {
  ...
  $effect(() => {
    const r = roomId;
    const source = new EventSource(`/api/events?room=${encodeURIComponent(r)}`);

    source.onopen = () => {
      if (isFirstOpen) { isFirstOpen = false; return; }
      fetchRecentImages(r).then((imgs) => (images = imgs));
    };
  ...
  $effect(() => {
    const r = roomId;
    if ($viewingUser) {
      fetchUserImages($viewingUser, r).then((imgs) => (userImages = imgs));
    } else {
      userImages = [];
    }
  });
