...
describe('SqliteImageRepository', () => {
  it('insertImage returns the created record with an id', () => {
    const repo = makeRepo();
    const r = repo.insertImage('uid1', 'uid1/test.webp');
  ...
  it('countImages returns total count', () => {
    const repo = makeRepo();
    repo.insertImage('uid1', 'p1');
    repo.insertImage('uid1', 'p2');
    repo.insertImage('uid2', 'p3');
  ...
  it('countImages filters by uid', () => {
    const repo = makeRepo();
    repo.insertImage('uid1', 'p1');
    repo.insertImage('uid2', 'p2');
    expect(repo.countImages({ uid: 'uid1' })).toBe(1);
  });

  it('getImages returns newest first by default', () => {
    const repo = makeRepo();
    const r1 = repo.insertImage('uid1', 'p1');
    const r2 = repo.insertImage('uid1', 'p2');
  ...
  it('deleteImage removes the row', () => {
    const repo = makeRepo();
    const r = repo.insertImage('uid1', 'p1');
