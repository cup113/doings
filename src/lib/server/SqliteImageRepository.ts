...
export class SqliteImageRepository implements ImageRepository {
  ...
  getImages(params: GetImagesParams): ImageRecord[] {
    let sql = 'SELECT id, uid, path, created_at FROM images WHERE 1=1';
  ...
  countImages(params: CountImagesParams): number {
    let sql = 'SELECT COUNT(*) as count FROM images';
    const args: SQLInputValue[] = [];
  ...
  getImageById(id: number): ImageRecord | null {
    const row = this.db.prepare('SELECT id, uid, path, created_at FROM images WHERE id = ?').get(id) as ImageRecord | undefined;
    return row ?? null;
  }

  insertImage(uid: string, path: string): ImageRecord {
    const stmt = this.db.prepare('INSERT INTO images (uid, path) VALUES (?, ?)');
    const { lastInsertRowid } = stmt.run(uid, path);
    const row = this.db.prepare('SELECT id, uid, path, created_at FROM images WHERE id = ?').get(Number(lastInsertRowid)) as unknown as ImageRecord;
    return row;
  }
